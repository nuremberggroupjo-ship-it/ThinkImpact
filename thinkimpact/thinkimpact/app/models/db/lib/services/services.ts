...
export const addNewService = async (newService: newService) => {
  const result = await pool.query<newService>(
    "insert into services (name_en,name_ar,description_en,description_ar,category_id) values ($1,$2,$3,$4,$5) returning *",
    [
      newService.name_en,
      newService.name_ar,
...
export const getAllServices = async () => {
  const result = await pool.query(
    "SELECT * FROM services INNER JOIN consulting ON services.category_id = consulting.id"
  );
  return result.rows;
};
...
export const editService = async (id: string, modifiedService: newService) => {
  ...
  if (isValidId.rows.length === 0) {
    ...
    const result = await pool.query<newService>(
      " update services  set name_en= coalesce ($2,name_en ), name_ar = coalesce ($3,name_ar ) ,description_en = coalesce ($4,description_en) ,description_ar = coalesce($5,description_ar),category_id= coalesce($6,category_id) where id= $1 returning * ",
      [
        id,
